"""
Submit PySpark job to EMR on EC2 using venv-based Python 3.12 environment.

Uses the same venv archive as EMR Serverless.
The only difference is the Spark config key prefix:
- EMR Serverless: spark.emr-serverless.driverEnv.*
- EMR on EC2:     spark.yarn.appMasterEnv.*
"""
...
S3_VENV_ARCHIVE = f"s3://{S3_BUCKET}/{S3_PREFIX}/libs/pyspark_venv.tar.gz"

CLUSTER_NAME = "emr-poc-ec2-venv"
...
def get_or_create_cluster(client):
    ...
    subnet_id = os.environ.get("EMR_SUBNET_ID", "subnet-02bbd41a14f0b75e1")
    ...
    resp = client.run_job_flow(
        ...
        Instances={
            ...
            "EmrManagedMasterSecurityGroup": os.environ.get("EMR_MASTER_SG", "sg-0ef7b638438dca093"),
            "EmrManagedSlaveSecurityGroup": os.environ.get("EMR_SLAVE_SG", "sg-0fb2a7b4cb230fc25"),
        },
...
def submit_step(client, cluster_id):
    step_name = f"poc-venv-{datetime.now().strftime('%Y%m%d-%H%M%S')}"

    args = [
        "spark-submit",
        "--deploy-mode", "cluster",
        "--archives", f"{S3_VENV_ARCHIVE}#environment",
        ...
        "--conf", "spark.yarn.appMasterEnv.PYTHONHOME=./environment",
...
def main():
    print("=" * 60)
    print("EMR on EC2 - Venv Package Submission (Python 3.12)")
