"""
Submit PySpark job to EMR Serverless using venv-based Python 3.12 environment.

The venv archive (pyspark_venv.tar.gz) contains:
- Python 3.12 interpreter (via --copies)
- Third-party packages (requests, numpy, pandas)
- Custom shared libraries (shared_libs/)

Spark uses the venv's Python binary as PYSPARK_PYTHON on all nodes.
"""
...
def get_or_create_application(client):
    app_name = f"emr-poc-venv-{RELEASE_LABEL.replace('emr-', '')}"
...
def submit_job(client, app_id, role_arn):
    job_name = f"poc-venv-{datetime.now().strftime('%Y%m%d-%H%M%S')}"

    spark_params = " ".join([
        f"--conf spark.archives={S3_VENV_ARCHIVE}#environment",
...
def main():
    print("=" * 60)
    print("EMR Serverless - Venv Package Submission (Python 3.12)")
